...
// Register fonts — must happen before any canvas drawing
try {
  ...
  console.warn('Font registration failed:', e.message, '— falling back to system fonts');
}
...
console.log('Claude AI:', ANT_KEY ? '✓ connected' : '✗ no key — text overlay disabled');
...
app.post('/api/mockup', async (req, res) => {
  ...
  try {
    ...
    // Save BG only (for interactive editor)
    const bgName = `bg-${Date.now()}.jpg`;
    const bgPath = path.join(__dirname, 'public/uploads', bgName);
    const bgCanvas = createCanvas(W, H);
    bgCanvas.getContext('2d').drawImage(bgImg, 0, 0, W, H);
    fs.writeFileSync(bgPath, bgCanvas.toBuffer('image/jpeg', { quality: 0.95 }));
 
    const finalUrl = `${req.protocol}://${req.get('host')}/uploads/${outName}`;
    const bgLocalUrl = `${req.protocol}://${req.get('host')}/uploads/${bgName}`;
    const designLocalUrl = `${req.protocol}://${req.get('host')}/uploads/${designFilename}`;
    console.log('[promo] done:', finalUrl);
    res.json({
      url: finalUrl,           // composited (fallback)
      bgUrl: bgLocalUrl,       // background only (for editor)
      designUrl: designLocalUrl, // design only (for editor)
      phonePos: phonePos,      // suggested position
      canvasW: W, canvasH: H,  // dimensions
...
app.post('/api/generate-text', async (req, res) => {
  if (!ant) return res.status(500).json({ error: 'ANTHROPIC_API_KEY not set in Railway env vars' });
 
  const { productName, productDesc, targetAudience, price, platform = 'instagram' } = req.body;
  if (!productName) return res.status(400).json({ error: 'Product name required' });
 
  const prompt = `You are a high-converting social media copywriter for female digital entrepreneurs.
 
Write promotional text for this product:
- Product: ${productName}
- Description: ${productDesc || 'a digital product'}
- Target audience: ${targetAudience || 'female entrepreneurs and content creators'}
- Price: ${price || 'not specified'}
- Platform: ${platform}
 
Write exactly 3 lines of copy:
1. HEADLINE: A powerful, bold hook (max 6 words, all caps, no punctuation)
2. SUBTEXT: One compelling supporting line (max 10 words, sentence case)
3. CTA: A short action line (max 6 words, no arrow, just the action)
